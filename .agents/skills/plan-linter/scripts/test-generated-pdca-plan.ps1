...
try {
    ...
    foreach($id in $stageIds){
        ...
        if($id-in@("${prefix}-000","${prefix}-001")){
            ...
            foreach($subtree in @($m.allowed_subtrees)){if($subtree-notmatch'^(docsplan|\.agents)(/|$)'){$errors.Add("${mp}: planejamento permite produto: $subtree")}}
        }
        if($m.stage_type-eq"REPLANNING"){
            ...
            foreach($subtree in @($m.allowed_subtrees)){if($subtree-notmatch'^(docsplan|\.agents)(/|$)'){$errors.Add("${mp}: REPLANNING permite produto: $subtree")}}
        }
