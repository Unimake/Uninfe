...
> **Decisões necessárias:** DEC-002, DEC-003, DEC-006 e DEC-007
> **Manifesto:** docsplan/nfeabi/plans/manifests/UAB-005.json
...
TaskNFeABIRecepcaoSinc, tratamento de `infRespTec` aprovado, XML assinado em processamento, Result/cStat, procNFeABI, fluxo/cleanup, rejeição e diagnóstico.
...
Task autorização, leitura/geração/processamento NFeABI, testes e exemplos sintéticos. O dossiê, o PDCA e as evidências desta etapa ficam restritos a `docsplan/nfeabi/`.

## 7. Contratos, invariantes e arquitetura

- Preservar literalmente os contratos aprovados: entrada `-nfeabi.xml`, retorno `-ret-nfeabi.xml`, erro `-ret-nfeabi.err` e processado autorizado `-procNFeABI.xml`.
- Para `infRespTec`, replicar a regra atual da NFGas: preservar quando presente; se ausente, criar quando qualquer um dos campos configurados existir e copiar os valores disponíveis; se todos estiverem vazios, manter ausente. Não adicionar validação de completude no UniNFe.
...
|---:|---|---|---:|---|---|
| 1 | build + testes de contexto para sucesso, rejeição e falhas entre movimentos | ENV-UNI-DEBUG | 15 min | proc/original/retorno/erro exatamente nos destinos aprovados | docsplan/nfeabi/plans/evidence/UAB-005/evidence/validation.txt |

## 12. Testes

Mockar DLL/retornos; falhas antes/depois de proc; digest/chave, cStat 100 e rejeição; `infRespTec` conforme decisão; nenhuma rede.
...
- Pasta: docsplan/nfeabi/plans/evidence/UAB-005/.
