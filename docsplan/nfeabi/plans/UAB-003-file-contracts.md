...
> **Decisões necessárias:** DEC-003, DEC-004, DEC-005 e DEC-007
> **Manifesto:** docsplan/nfeabi/plans/manifests/UAB-003.json
...
Components/enums/extensões, detecção Processar, settings/UI somente se DEC-005 aprovar, e testes. O dossiê, o PDCA e as evidências desta etapa ficam restritos a `docsplan/nfeabi/`.

## 7. Contratos, invariantes e arquitetura

- Autorização: entrada `-nfeabi.xml`, retorno `-ret-nfeabi.xml`, erro `-ret-nfeabi.err` e processado autorizado `-procNFeABI.xml`.
- Status: reutilizar `-ped-sta.xml`, `-sta.xml` e `-sta.err`, distinguindo NFeABI pela raiz `consStatServNFeABI`; não criar sufixo específico de Status.
- Acrescentar `TipoAplicativo.NFeABI = 17` sem renumerar valores existentes; expor “NF-e ABI” nos seletores e fluxos de configuração atuais e no modo `Todos`, seguindo o mesmo conjunto de call sites da NFGas e sem tela nova.
...
|---:|---|---|---:|---|---|
| 1 | build projetos afetados + testes de detecção/extensões | ENV-UNI-DEBUG | 15 min | raízes roteadas e contratos literais congelados | docsplan/nfeabi/plans/evidence/UAB-003/evidence/validation.txt |
...
- Pasta: docsplan/nfeabi/plans/evidence/UAB-003/.
