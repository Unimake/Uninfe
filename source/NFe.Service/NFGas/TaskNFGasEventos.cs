...
{
    public class TaskNFGasEventos : TaskAbst
    {
        public TaskNFGasEventos(string arquivo)
        {
            Servico = Servicos.NFGasRecepcaoEvento;
        ...
        public override void Execute()
        {
            ...
            try
            {
                ...
                var recepcaoEvento = new Unimake.Business.DFe.Servicos.NFGas.RecepcaoEvento(xml, configuracao);
        ...
        private void LerRetornoEvento(int emp, RetEventoNFGas retornoEventoNFGas)
        {
            var docEventoOriginal = ConteudoXML;
            var autorizou = false;

            var versao = retornoEventoNFGas.Versao;
            var infEvento = retornoEventoNFGas.InfEvento;
            ...
            if (cStatCons == 134 || cStatCons == 135 || cStatCons == 136)
            {
                ...
                var idEventoRetorno = TpcnResources.ID.ToString() + tpEvento + chNFGas + nSeqEvento.ToString((idEventoOriginal.Length <= 54 ? "00" : "000"));
                ...
                foreach (XmlNode env in docEventoOriginal.GetElementsByTagName("infEvento"))
                {
                    if (idEventoOriginal == idEventoRetorno)
                    {
                        ...
                        oGerarXML.XmlDistEventoNFGas(emp, chNFGas, nSeqEvento.ToString((idEventoOriginal.Length <= 54 ? "00" : "000")), Convert.ToInt32(tpEvento),
                            env.ParentNode.OuterXml, retornoEventoNFGas.GerarXML().OuterXml, Functions.GetDateTime(infEvento.DhRegEvento), true, versao);
                        ...
                        //        catch (Exception ex)
                        //        {
                        //            Auxiliar.WriteLog("TaskNFGasEventos: " + ex.Message, false);
                        //        }
                        //        break;
                        //}
            ...
            else
            {
                if (Empresas.Configuracoes[emp].DocumentosRejeitados)
                {
                    ...
                    sendMessageToWhatsApp.AlertNotification("Rejeição: " + Convert.ToInt32(cStatCons).ToString("000") + "-" + xMotivo.Trim(), "UNINFE - Evento da NFGas rejeitado");
                }
            }
