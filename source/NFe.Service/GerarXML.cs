...
namespace NFe.Service
{
    ...
    public class GerarXML
    {
        ...
        public void Consulta(TipoAplicativo tipoAplicativo, string Arquivo, int tpAmb, int tpEmis, string chNFe, string versao)
        {
            ...
            switch (tipoAplicativo)
            {
                ...
                case TipoAplicativo.Nfe:
                    xmlDados = ConsultaNFe(tpAmb, chNFe, versao);
                    break;
        ...
        public string StatusServico(TipoAplicativo servico, int tpEmis, int cUF, int amb, string versao)
        {
            ...
            switch (servico)
            {
                ...
                case TipoAplicativo.NF3e:
                    StatusServicoNF3e(arquivoSaida, amb, tpEmis, cUF, versao);
                    break;
                ...
                case TipoAplicativo.BPe:
                    StatusServicoBPe(arquivoSaida, amb, tpEmis, cUF, versao);
                    break;
        ...
        public void StatusServicoCTe(string pArquivo, int tpAmb, int tpEmis, int cUF, string versao)
        {
            ...
            if (versao == "4.00" && tpEmis != 1 && pArquivo.ToLower().IndexOf(Empresas.Configuracoes[EmpIndex].PastaValidar.ToLower()) == -1)
            {
        ...
        /// <summary>
        /// Gera o XML de consulta status do serviço da NF3e
        /// </summary>
        /// <param name="pArquivo">Caminho e nome do arquivo que é para ser gerado</param>
        /// <param name="tpAmb">Ambiente da consulta</param>
        /// <param name="tpEmis">Tipo de emissão da consulta</param>
        ...
        public void StatusServicoNF3e(string pArquivo, int tpAmb, int tpEmis, int cUF, string versao)
        {
            ...
            var xmlNode = doc.GetElementsByTagName("consStatServNF3e")[0];
            xmlNode.AppendChild(CriaElemento(doc, TpcnResources.cUF.ToString(), cUF.ToString(), NFeStrConstants.NAME_SPACE_NF3E));

            if (tpEmis != 1 && pArquivo.ToLower().IndexOf(Empresas.Configuracoes[EmpIndex].PastaValidar.ToLower()) == -1)
            {
                xmlNode.AppendChild(CriaElemento(doc, TpcnResources.tpEmis.ToString(), tpEmis.ToString(), NFeStrConstants.NAME_SPACE_NF3E));
                doc.AppendChild(xmlNode);
            }
        ...
        public void StatusServicoNFCom(string pArquivo, int tpAmb, int tpEmis, int cUF, string versao)
        {
            ...
            var xmlNode = doc.GetElementsByTagName("consStatServNFCom")[0];
            xmlNode.AppendChild(CriaElemento(doc, TpcnResources.cUF.ToString(), cUF.ToString(), NFeStrConstants.NAME_SPACE_NFCOM));

            if (tpEmis != 1 && pArquivo.ToLower().IndexOf(Empresas.Configuracoes[EmpIndex].PastaValidar.ToLower()) == -1)
            {
                xmlNode.AppendChild(CriaElemento(doc, TpcnResources.tpEmis.ToString(), tpEmis.ToString(), NFeStrConstants.NAME_SPACE_NFCOM));
                doc.AppendChild(xmlNode);
            }
        ...
        #region StatusServicoBPe()

        /// <summary>
        /// Gera o XML de consulta status do serviço do BPe
        /// </summary>
        /// <param name="pArquivo">Caminho e nome do arquivo que é para ser gerado</param>
        /// <param name="tpAmb">Ambiente da consulta</param>
        /// <param name="tpEmis">Tipo de emissão da consulta</param>
        /// <param name="cUF">Estado para a consulta</param>
        /// <param name="versao">Versão do schema do XML</param>
        public void StatusServicoBPe(string pArquivo, int tpAmb, int tpEmis, int cUF, string versao)
        {
            ...
            var xmlNode = doc.GetElementsByTagName("consStatServBPe")[0];
            xmlNode.AppendChild(CriaElemento(doc, TpcnResources.cUF.ToString(), cUF.ToString(), NFeStrConstants.NAME_SPACE_BPE));

            if (tpEmis > 0)
            {
                xmlNode.AppendChild(CriaElemento(doc, TpcnResources.tpEmis.ToString(), tpEmis.ToString(), NFeStrConstants.NAME_SPACE_BPE));
            }

            GravarArquivoParaEnvio(pArquivo, doc.OuterXml);
        }

        #endregion StatusServicoBPe()
            ...
            switch (Servico)
            {
                                ...
                                if (retEnviNFeElemento != null)   //danasa 23-9-2009
                                {
                                    ...
                                    if (infProtList != null)
                                    {
                                        if (infProtList.Count > 0)
                                        {
                                            ...
                                            {
                                                var chNFe = Functions.LerTag(infProtElemento, TpcnResources.chNFe.ToString());
                                                ...
                                                ConteudoRetorno += Functions.LerTag(infProtElemento, TpcnResources.digVal.ToString());
                                                ConteudoRetorno += Functions.LerTag(infProtElemento, TpcnResources.cStat.ToString());
                                                ConteudoRetorno += Functions.LerTag(infProtElemento, TpcnResources.xMotivo.ToString());
                                            }
                                        }
                                    }
                ...
                case Servicos.NFePedidoSituacaoLote:
                    {
                        ...
                        if (retConsReciNFeList != null) //danasa 23-9-2009
                        {
                            if (retConsReciNFeList.Count > 0)   //danasa 23-9-2009
                            {
                                ...
                                {
                                    ConteudoRetorno += Functions.LerTag(retConsReciNFeElemento, TpcnResources.nRec.ToString());
                                    ConteudoRetorno += Functions.LerTag(retConsReciNFeElemento, TpcnResources.cStat.ToString());
                                    ConteudoRetorno += Functions.LerTag(retConsReciNFeElemento, TpcnResources.xMotivo.ToString());
                                    ConteudoRetorno += "\r\n";
                                    ...
                                    if (retInutElemento.ChildNodes.Count > 0)
                                    {
                                        ...
                                        if (infInutList != null)
                                        {
                                            foreach (XmlNode infInutNode in infInutList)
                                            {
                                                ...
                                                ConteudoRetorno += Functions.LerTag(infInutElemento, TpcnResources.tpAmb.ToString());
                                                ConteudoRetorno += Functions.LerTag(infInutElemento, TpcnResources.cStat.ToString());
                                                ConteudoRetorno += Functions.LerTag(infInutElemento, TpcnResources.xMotivo.ToString());
                                                ConteudoRetorno += Functions.LerTag(infInutElemento, TpcnResources.cUF.ToString());
                                                ConteudoRetorno += "\r\n";
                                            }
                                        }
                                    }
                                    ...
                                    if (retConsSitElemento.ChildNodes.Count > 0)
                                    {
                                        ...
                                        if (infConsSitList != null)
                                        {
                                            foreach (XmlNode infConsSitNode in infConsSitList)
                                            {
                                                ...
                                                var nProt = Functions.LerTag(infConsSitElemento, TpcnResources.nProt.ToString());

                                                ConteudoRetorno += Functions.LerTag(infConsSitElemento, TpcnResources.tpAmb.ToString());
                                                ConteudoRetorno += Functions.LerTag(infConsSitElemento, TpcnResources.cStat.ToString());
                                                ConteudoRetorno += Functions.LerTag(infConsSitElemento, TpcnResources.xMotivo.ToString());
                                                ConteudoRetorno += Functions.LerTag(infConsSitElemento, TpcnResources.cUF.ToString());
                                                ConteudoRetorno += Functions.LerTag(infConsSitElemento, TpcnResources.dhRecbto.ToString());
                                                ConteudoRetorno += (string.IsNullOrWhiteSpace(nProt) ? "0;" : nProt);
                                                ConteudoRetorno += "\r\n";
                                            }
                                        }
                ...
                case Servicos.NFeConsultaStatusServico:   //danasa 19-9-2009
                    {
                        ...
                        if (retConsStatServList != null)
                        {
                            if (retConsStatServList.Count > 0)
                            {
                                ...
                                {
                                    ConteudoRetorno += Functions.LerTag(retConsStatServElemento, TpcnResources.tpAmb.ToString());
                                    ConteudoRetorno += Functions.LerTag(retConsStatServElemento, TpcnResources.cStat.ToString());
                                    ConteudoRetorno += Functions.LerTag(retConsStatServElemento, TpcnResources.xMotivo.ToString());
                                    ConteudoRetorno += Functions.LerTag(retConsStatServElemento, TpcnResources.cUF.ToString());
                                    ConteudoRetorno += Functions.LerTag(retConsStatServElemento, TpcnResources.dhRecbto.ToString());
                ...
                case Servicos.CTeDistribuicaoDFe:
                    ...
                    foreach (XmlNode ret1Node in envEventoList)
                    {
                        ...
                        ConteudoRetorno += Functions.LerTag(ret1Elemento, TpcnResources.tpAmb.ToString());
                        ConteudoRetorno += Functions.LerTag(ret1Elemento, TpcnResources.verAplic.ToString());
                        ConteudoRetorno += Functions.LerTag(ret1Elemento, TpcnResources.cStat.ToString());
                        ConteudoRetorno += Functions.LerTag(ret1Elemento, TpcnResources.xMotivo.ToString());
                        ConteudoRetorno += Functions.LerTag(ret1Elemento, TpcnResources.dhResp.ToString());
                        ConteudoRetorno += Functions.LerTag(ret1Elemento, TpcnResources.ultNSU.ToString());
                        ...
                        foreach (XmlNode ret in ret1List)
                        {
                            for (var n = 0; n < ret.ChildNodes.Count; ++n)
                            {
                                if (ret.ChildNodes[n].Name.Equals("docZip"))
                                {
                                    ...
                                    if (ret.ChildNodes[n].Attributes["NSU"] == null)
                                    {
                                        NSU = "000000000000000";
                                    }
                                    else
                                    ...
                                    if (ret.ChildNodes[n].Attributes["schema"].InnerText.StartsWith("resEvento"))
                                    {
                                        ...
                                        chNFe = Functions.LerTag(ret1, TpcnResources.chNFe.ToString(), false);
                                        var nSeqEvento = Convert.ToInt32("0" + Functions.LerTag(ret1, TpcnResources.nSeqEvento.ToString(), false));
                                        var tpEvento = (ConvertTxt.tpEventos)Convert.ToInt32("0" + Functions.LerTag(ret1, TpcnResources.tpEvento.ToString(), false));
                                    ...
                                    else if (ret.ChildNodes[n].Attributes["schema"].InnerText.StartsWith("procEventoCTe"))
                                    {
                                        ...
                                        var ret1 = (XmlElement)envres.Item(0);
                                        chNFe = Functions.LerTag(ret1, TpcnResources.chCTe.ToString(), false);
                                        var nSeqEvento = Convert.ToInt32("0" + Functions.LerTag(ret1, TpcnResources.nSeqEvento.ToString(), false));
                                        var tpEvento = (ConvertTxt.tpEventos)Convert.ToInt32("0" + Functions.LerTag(ret1, TpcnResources.tpEvento.ToString(), false));

                                        var versaoSchemaCTe = "3.00";
                                        var listEventoCTe = docres.GetElementsByTagName("eventoCTe");
                                        ...
                                        ConteudoRetorno += "procEventoCTe;" + NSU + ";" + chNFe + ";" + tpEvento.ToString() + ";" + nSeqEvento.ToString((versaoSchemaCTe.Equals("3.00") ? "00" : "000"));
                                    }
                                        ...
                                        if (infCCeList != null)
                                        {
                                            foreach (XmlNode infCCeNode in infCCeList)
                                            {
                                                foreach (XmlNode infCCeNode2 in infCCeNode.ChildNodes)
                                                {
                                                    ...
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.tpAmb.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.cOrgao.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.cStat.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.xMotivo.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.chNFe.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.tpEvento.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.xEvento.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.nSeqEvento.ToString());
                                                    var FCNPJCPF = Functions.LerTag(infCCeElemento, TpcnResources.CNPJDest.ToString(), false);
                                                    if (string.IsNullOrEmpty(FCNPJCPF))
                                                    {
                                                        FCNPJCPF = Functions.LerTag(infCCeElemento, TpcnResources.CPFDest.ToString(), false);
                                                    }

                                                    ConteudoRetorno += FCNPJCPF + ";";
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.dhRegEvento.ToString());
                                                    ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.nProt.ToString());

                                                    switch (EnumHelper.StringToEnum<NFe.ConvertTxt.tpEventos>(Functions.LerTag(infCCeElemento, TpcnResources.tpEvento.ToString(), false)))
                                                    {
                                                        case ConvertTxt.tpEventos.tpEvEPEC:
                                                            {
                                                                ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.cOrgaoAutor.ToString());
                                                        ...
                                                        case ConvertTxt.tpEventos.tpEvCancPedProrrogacao_ICMS_2:
                                                            ConteudoRetorno += Functions.LerTag(infCCeElemento, TpcnResources.emailDest.ToString());
                                                            break;
                                                    }
                                                    ConteudoRetorno += "\r\n";
                                                }
        ...
        public string XmlDistCTe(string arqCTe, string protCTe, string versao)  //danasa 11-4-2012
        {
            ...
            try
            {
                ...
                if (File.Exists(arqCTe))
                {
                    ...
                    XmlNodeList CTeList = null;

                    if (doc.GetElementsByTagName(tipo.ToUpper() + "e").Count > 0)
                    {
                        CTeList = doc.GetElementsByTagName(tipo.ToUpper() + "e");
                    }
                    else
                    {
                        CTeList = doc.GetElementsByTagName(tipo.ToUpper() + "eSimp");
                    }
        ...
        /// <summary>
        /// Criar o arquivo XML de distribuição das NF3e's com o protocolo de autorização anexado
        /// </summary>
        ...
        {
            var nomeArqProcNF3e = string.Empty;
            var emp = EmpIndex;
            StreamWriter swProc = null;

            try
            {
                if (File.Exists(arqNF3e))
                {
                    var tipo = "nf3";
                    ...
                    var NF3eList = doc.GetElementsByTagName(tipo.ToUpper() + "e");
                    var NF3eNode = NF3eList[0];
                    var conteudoNF3e = NF3eNode.OuterXml;

                    var xmlProcNF3e = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
                        "<" + tipo + "eProc xmlns=\"" + NFeStrConstants.NAME_SPACE_NF3E + "\" versao=\"" + versao + "\">" +
                        conteudoNF3e +
                        protNF3e +
                        "</" + tipo + "eProc>";

                    nomeArqProcNF3e = Empresas.Configuracoes[emp].PastaXmlEnviado + "\\" +
                        PastaEnviados.EmProcessamento.ToString() + "\\" + Functions.ExtrairNomeArq(arqNF3e, Propriedade.Extensao(Propriedade.TipoEnvio.NF3e).EnvioXML) + extensao;

                    swProc = File.CreateText(nomeArqProcNF3e);
                    swProc.Write(xmlProcNF3e);
                }
            }
            finally
            {
            ...
            return nomeArqProcNF3e;
        }

        #endregion XmlDistNF3e()

        #region XmlDistNFCom()

        /// <summary>
        /// Criar o arquivo XML de distribuição das NFCom's com o protocolo de autorização anexado
        /// </summary>
        ...
        {
            var nomeArqProcNFCom = string.Empty;
            var emp = EmpIndex;
            StreamWriter swProc = null;

            try
            {
                if (File.Exists(arqNFCom))
                {
                    var tipo = "nfc";
                    ...
                    var NFComList = doc.GetElementsByTagName(tipo.ToUpper() + "om");
                    var NFComNode = NFComList[0];
                    var conteudoNFCom = NFComNode.OuterXml;

                    var xmlProcNFCom = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
                        "<" + tipo + "omProc xmlns=\"" + NFeStrConstants.NAME_SPACE_NFCOM + "\" versao=\"" + versao + "\">" +
                        conteudoNFCom +
                        protNFCom +
                        "</" + tipo + "omProc>";

                    nomeArqProcNFCom = Empresas.Configuracoes[emp].PastaXmlEnviado + "\\" +
                        PastaEnviados.EmProcessamento.ToString() + "\\" + Functions.ExtrairNomeArq(arqNFCom, Propriedade.Extensao(Propriedade.TipoEnvio.NFCom).EnvioXML) + extensao;

                    swProc = File.CreateText(nomeArqProcNFCom);
                    swProc.Write(xmlProcNFCom);
                }
            }
            finally
            {
        ...
        /// <summary>
        /// Criar o arquivo XML de distribuição do BPe com o protocolo de autorização anexado
        /// </summary>
        /// <param name="arqBPe">Nome arquivo do XML do BPe</param>
        /// <param name="protBPe">String contendo a parte do XML do protocolo a ser anexado</param>
        /// <param name="extensao">Extensão que será utilizada na criação do arquivo na pasta</param>
        /// <param name="versao">Versão do SCHEMA do XML</param>
        /// <returns></returns>
        public string XmlDistBPe(string arqBPe, string protBPe, string extensao, string versao)
        {
            return XmlDistBPeBase(arqBPe, protBPe, extensao, versao, "BPe", "BPeProc", Propriedade.TipoEnvio.BPe);
        }

        #endregion XmlDistBPe()

        #region XmlDistBPeTM()

        /// <summary>
        /// Criar o arquivo XML de distribuição do BPe TM com o protocolo de autorização anexado
        /// </summary>
        /// <param name="arqBPeTM">Nome arquivo do XML do BPe TM</param>
        /// <param name="protBPe">String contendo a parte do XML do protocolo a ser anexado</param>
        /// <param name="extensao">Extensão que será utilizada na criação do arquivo na pasta</param>
        /// <param name="versao">Versão do SCHEMA do XML</param>
        /// <returns></returns>
        public string XmlDistBPeTM(string arqBPeTM, string protBPe, string extensao, string versao)
        {
            return XmlDistBPeBase(arqBPeTM, protBPe, extensao, versao, "BPeTM", "BPeTMProc", Propriedade.TipoEnvio.BPeTM);
        }

        #endregion XmlDistBPeTM()

        #region XmlDistBPeTA()

        /// <summary>
        /// Criar o arquivo XML de distribuição do BPe TA com o protocolo de autorização anexado
        /// </summary>
        /// <param name="arqBPeTA">Nome arquivo do XML do BPe TA</param>
        /// <param name="protBPe">String contendo a parte do XML do protocolo a ser anexado</param>
        /// <param name="extensao">Extensão que será utilizada na criação do arquivo na pasta</param>
        /// <param name="versao">Versão do SCHEMA do XML</param>
        /// <returns></returns>
        public string XmlDistBPeTA(string arqBPeTA, string protBPe, string extensao, string versao)
        {
            return XmlDistBPeBase(arqBPeTA, protBPe, extensao, versao, "BPeTA", "bpeTAProc", Propriedade.TipoEnvio.BPeTA);
        }

        #endregion XmlDistBPeTA()
        #region XmlDistBPeBase()

        private string XmlDistBPeBase(string arqBPe, string protBPe, string extensao, string versao, string tagDocumento, string tagProc, Propriedade.TipoEnvio tipoEnvio)
        {
            var nomeArqProcBPe = string.Empty;
            var emp = EmpIndex;
            StreamWriter swProc = null;

            try
            {
                if (File.Exists(arqBPe))
                {
                    var doc = new XmlDocument();
                    doc.Load(arqBPe);

                    var bpeList = doc.GetElementsByTagName(tagDocumento);
                    var bpeNode = bpeList[0];
                    var conteudoBPe = bpeNode.OuterXml;
                    var xmlProcBPe = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
                        "<" + tagProc + " xmlns=\"" + NFeStrConstants.NAME_SPACE_BPE + "\" versao=\"" + versao + "\">" +
                        conteudoBPe +
                        protBPe +
                        "</" + tagProc + ">";

                    nomeArqProcBPe = Empresas.Configuracoes[emp].PastaXmlEnviado + "\\" +
                        PastaEnviados.EmProcessamento.ToString() + "\\" + Functions.ExtrairNomeArq(arqBPe, Propriedade.Extensao(tipoEnvio).EnvioXML) + extensao;

                    swProc = File.CreateText(nomeArqProcBPe);
                    swProc.Write(xmlProcBPe);
                }
            }
            finally
            {
            ...
            return nomeArqProcBPe;
        }

        #endregion XmlDistBPeBase()
        ...
        public void EnvioEvento(string pArquivo, DadosenvEvento dadosEnvEvento)
        {
            ...
            envEvento.Attributes.Append(CriaAttribute(doc, TpcnResources.versao.ToString(), NFe.ConvertTxt.versoes.VersaoXMLEvento));
            envEvento.Attributes.Append(CriaAttribute(doc, TpcnResources.xmlns.ToString(), NFeStrConstants.NAME_SPACE_NFE));
            envEvento.AppendChild(CriaElemento(doc, TpcnResources.idLote.ToString(), dadosEnvEvento.idLote));
            foreach (var evento in dadosEnvEvento.eventos)
            {
                XmlNode nodeEvento = doc.CreateElement("evento");
                nodeEvento.Attributes.Append(CriaAttribute(doc, TpcnResources.versao.ToString(), dadosEnvEvento.versao));
                nodeEvento.Attributes.Append(CriaAttribute(doc, TpcnResources.xmlns.ToString(), NFeStrConstants.NAME_SPACE_NFE));

                XmlNode infEvento = doc.CreateElement("infEvento");
                infEvento.Attributes.Append(CriaAttribute(doc, TpcnResources.Id.ToString(), evento.Id));
                infEvento.AppendChild(CriaElemento(doc, TpcnResources.cOrgao.ToString(), evento.cOrgao.ToString()));
                infEvento.AppendChild(CriaElemento(doc, TpcnResources.tpAmb.ToString(), evento.tpAmb.ToString()));
                if (evento.tpEmis != 0)
                {
                    infEvento.AppendChild(CriaElemento(doc, TpcnResources.tpEmis.ToString(), evento.tpEmis.ToString()));
                }

                if (!string.IsNullOrEmpty(evento.CNPJ))
                {
                    infEvento.AppendChild(CriaElemento(doc, TpcnResources.CNPJ.ToString(), evento.CNPJ));
                }
                else
                {
                    infEvento.AppendChild(CriaElemento(doc, TpcnResources.CPF.ToString(), evento.CPF));
                }
                ...
                infEvento.AppendChild(CriaElemento(doc, TpcnResources.dhEvento.ToString(), evento.dhEvento));
                infEvento.AppendChild(CriaElemento(doc, TpcnResources.tpEvento.ToString(), evento.tpEvento));
                infEvento.AppendChild(CriaElemento(doc, TpcnResources.nSeqEvento.ToString(), evento.nSeqEvento.ToString()));
                infEvento.AppendChild(CriaElemento(doc, TpcnResources.verEvento.ToString(), evento.verEvento));
                ...
                detEvento.Attributes.Append(CriaAttribute(doc, TpcnResources.versao.ToString(), "1.00"));
                detEvento.AppendChild(CriaElemento(doc, TpcnResources.descEvento.ToString(), evento.descEvento.Trim()));

                switch (EnumHelper.StringToEnum<NFe.ConvertTxt.tpEventos>(evento.tpEvento))
                {
                    case ConvertTxt.tpEventos.tpEvPedProrrogacao_ICMS_1:
                    case ConvertTxt.tpEventos.tpEvPedProrrogacao_ICMS_2:
                        {
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.nProt.ToString(), evento.nProt));
                            foreach (var ppICMS in evento.prorrogacaoICMS)
                            {
                                XmlNode itemPedido = doc.CreateElement(TpcnResources.itemPedido.ToString());
                                itemPedido.Attributes.Append(CriaAttribute(doc, TpcnResources.numItem.ToString(), ppICMS.numItem));
                                itemPedido.AppendChild(CriaElemento(doc, TpcnResources.qtdeItem.ToString(), ppICMS.qtdeItem.Trim()));
                    ...
                    case ConvertTxt.tpEventos.tpEvCancPedProrrogacao_ICMS_2:
                        {
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.idPedidoCancelado.ToString(), evento.idPedidoCancelado));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.nProt.ToString(), evento.nProt));
                        }
                        break;
                    ...
                    case ConvertTxt.tpEventos.tpEvFiscoRespCancPedProrrogacao_ICMS_2:
                        {
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.idPedido.ToString(), evento.idPedido));
                            switch (EnumHelper.StringToEnum<NFe.ConvertTxt.tpEventos>(evento.tpEvento))
                            {
                                case ConvertTxt.tpEventos.tpEvFiscoRespCancPedProrrogacao_ICMS_1:
                                case ConvertTxt.tpEventos.tpEvFiscoRespCancPedProrrogacao_ICMS_2:
                                    {
                                        XmlNode respCancPedido = doc.CreateElement(TpcnResources.respCancPedido.ToString());
                                        respCancPedido.AppendChild(CriaElemento(doc, TpcnResources.statCancPedido.ToString(), evento.respCancPedido.statCancPedido.ToString()));
                                        respCancPedido.AppendChild(CriaElemento(doc, TpcnResources.justStatus.ToString(), evento.respCancPedido.justStatus.ToString("00")));
                                        if (!string.IsNullOrEmpty(evento.respCancPedido.justStaOutra))
                                        {
                                            respCancPedido.AppendChild(CriaElemento(doc, TpcnResources.justStaOutra.ToString(), evento.respCancPedido.justStaOutra));
                                        }
                                ...
                                default:
                                    {
                                        XmlNode respPedido = doc.CreateElement(TpcnResources.respPedido.ToString());
                                        respPedido.AppendChild(CriaElemento(doc, TpcnResources.statPrazo.ToString(), evento.respPedido.statPrazo));

                                        foreach (var ipt in evento.respPedido.itemPedido)
                                        {
                                            XmlNode itemPedido = doc.CreateElement(TpcnResources.itemPedido.ToString());
                                            itemPedido.Attributes.Append(CriaAttribute(doc, TpcnResources.numItem.ToString(), ipt.numItem.ToString()));
                                            itemPedido.AppendChild(CriaElemento(doc, TpcnResources.statPedido.ToString(), ipt.statPedido.ToString()));
                                            itemPedido.AppendChild(CriaElemento(doc, TpcnResources.justStatus.ToString(), ipt.justStatus.ToString()));
                                            if (!string.IsNullOrEmpty(ipt.justStaOutra))
                                            {
                                                itemPedido.AppendChild(CriaElemento(doc, TpcnResources.justStaOutra.ToString(), ipt.justStaOutra));
                                            }
                    ...
                    case ConvertTxt.tpEventos.tpEvCCe:
                        {
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.xCorrecao.ToString(), evento.xCorrecao.Trim()));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.xCondUso.ToString(), evento.xCondUso.Trim()));
                        }
                        break;
                    ...
                    case ConvertTxt.tpEventos.tpEvCancelamentoNFe:
                        {
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.nProt.ToString(), evento.nProt));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.xJust.ToString(), evento.xJust.Trim()));
                        }
                        break;
                    ...
                    case ConvertTxt.tpEventos.tpEvCancelamentoSubstituicaoNFCe:
                        {
                            ...
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.verAplic.ToString(), evento.cancelamentoSubstituicao.verAplic.Trim()));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.nProt.ToString(), evento.nProt));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.xJust.ToString(), evento.xJust.Trim()));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.chNFeRef.ToString(), evento.cancelamentoSubstituicao.chNFeRef));
                        }
                        break;
                    ...
                    case ConvertTxt.tpEventos.tpEvEPEC:
                        {
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.cOrgaoAutor.ToString(), evento.epec.cOrgaoAutor.ToString()));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.tpAutor.ToString(), ((int)evento.epec.tpAutor).ToString()));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.verAplic.ToString(), evento.epec.verAplic.Trim()));
                            ...
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.dhEmi.ToString(), evento.epec.dhEmi));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.tpNF.ToString(), ((int)evento.epec.tpNF).ToString()));
                            detEvento.AppendChild(CriaElemento(doc, TpcnResources.IE.ToString(), evento.epec.IE));

                            XmlNode destEvento = doc.CreateElement("dest");
                            destEvento.AppendChild(CriaElemento(doc, TpcnResources.UF.ToString(), evento.epec.dest.UF));
                            if (!string.IsNullOrEmpty(evento.epec.dest.idEstrangeiro) || evento.epec.dest.UF.Equals("EX"))
                            {
                                destEvento.AppendChild(CriaElemento(doc, TpcnResources.idEstrangeiro.ToString(), evento.epec.dest.idEstrangeiro));
                            }
                            else
                                if (!string.IsNullOrEmpty(evento.epec.dest.CNPJ))
                            {
                                destEvento.AppendChild(CriaElemento(doc, TpcnResources.CNPJ.ToString(), evento.epec.dest.CNPJ));
                            }
                            else
                            {
                                destEvento.AppendChild(CriaElemento(doc, TpcnResources.CPF.ToString(), evento.epec.dest.CPF));
                            }

                            if (!string.IsNullOrEmpty(evento.epec.dest.IE) && !evento.epec.dest.IE.Equals("ISENTO"))
                            {
                                destEvento.AppendChild(CriaElemento(doc, TpcnResources.IE.ToString(), evento.epec.dest.IE));
                            }

                            switch (evento.chNFe.Substring(20, 2))
                            {
                                case "55":
                                    destEvento.AppendChild(CriaElemento(doc, TpcnResources.vNF.ToString(), evento.epec.dest.vNF.ToString("0.00").Replace(",", ".")));
                                    destEvento.AppendChild(CriaElemento(doc, TpcnResources.vICMS.ToString(), evento.epec.dest.vICMS.ToString("0.00").Replace(",", ".")));
                                    destEvento.AppendChild(CriaElemento(doc, TpcnResources.vST.ToString(), evento.epec.dest.vST.ToString("0.00").Replace(",", ".")));
                                ...
                                case "65":
                                    ...
                                    detEvento.AppendChild(CriaElemento(doc, TpcnResources.vNF.ToString(), evento.epec.dest.vNF.ToString("0.00").Replace(",", ".")));
                                    detEvento.AppendChild(CriaElemento(doc, TpcnResources.vICMS.ToString(), evento.epec.dest.vICMS.ToString("0.00").Replace(",", ".")));
                                    break;
                            }
                        }
                        break;

                    case ConvertTxt.tpEventos.tpEvConciliacaoFinanceiraNFe:
                        detEvento.AppendChild(CriaElemento(doc, TpcnResources.verAplic.ToString(), evento.verAplic.ToString()));

                        foreach (var element in evento.detPag)
                        {
                            XmlNode detPag = doc.CreateElement(TpcnResources.detPag.ToString());
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.indPag.ToString(), element.IndPag));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.tPag.ToString(), element.TPag));

                            if (!string.IsNullOrEmpty(element.XPag))
                            {
                                detPag.AppendChild(CriaElemento(doc, TpcnResources.xPag.ToString(), element.XPag));
                            }

                            detPag.AppendChild(CriaElemento(doc, TpcnResources.vPag.ToString(), element.VPag.ToString("0.00").Replace(",", ".")));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.dPag.ToString(), element.DPag));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.CNPJPag.ToString(), element.CNPJPag));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.UFPag.ToString(), element.UFPag));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.CNPJIF.ToString(), element.CNPJIF));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.tBand.ToString(), element.TBand));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.cAut.ToString(), element.CAut));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.CNPJReceb.ToString(), element.CNPJReceb));
                            detPag.AppendChild(CriaElemento(doc, TpcnResources.UFReceb.ToString(), element.UFReceb));
                    ...
                    case ConvertTxt.tpEventos.tpEvCancelamentoConciliacaoFinanceiraNFe:
                        ...
                        detEvento.AppendChild(CriaElemento(doc, TpcnResources.verAplic.ToString(), evento.verAplic.ToString()));
                        detEvento.AppendChild(CriaElemento(doc, TpcnResources.nProtEvento.ToString(), evento.cancelamentoConciliacaoFinanceira.NProtEvento.ToString()));
                        break;
                }
        ...
        public void RecepcaoDFe(string arquivo, string value) => GravarArquivoParaEnvio(arquivo, value, true);
        ...
        public void XmlDistEvento(int emp, string vStrXmlRetorno)
        {
            ...
            if (retprocEventoNFeList != null)
            {
                foreach (XmlNode retConsSitNode1 in retprocEventoNFeList)
                {
                    ...
                    {
                        var tpEvento = EnumHelper.StringToEnum<NFe.ConvertTxt.tpEventos>(((XmlElement)retConsSitNode1).GetElementsByTagName(TpcnResources.tpEvento.ToString())[0].InnerText);
                        if (tpEvento != ConvertTxt.tpEventos.tpEvEPEC)
                        {
                            var chNFe = ((XmlElement)retConsSitNode1).GetElementsByTagName(TpcnResources.chNFe.ToString())[0].InnerText;
            ...
            if (!string.IsNullOrEmpty(filenameBackup))
            {
                ...
                if (vePasta)
                {
                    ...
                    if (!string.IsNullOrEmpty(folderNFe))
                    {
                        ...
                        filenameBackup += Path.Combine(folderNFe.Replace(Empresas.Configuracoes[emp].PastaXmlEnviado, ""), Path.GetFileName(filenameToWrite));
                    }
                    else
                    {
        ...
        public void XmlDistEventoMDFe(int emp, string strXmlRetorno)
        {
            ...
            if (retprocEventoNFeList != null)
            {
                foreach (XmlNode retConsSitNode1 in retprocEventoNFeList)
                {
                    ...
                    if (cStat == "134" || cStat == "135" || cStat == "136")
                    {
                        var chNFe = ((XmlElement)retConsSitNode1).GetElementsByTagName(TpcnResources.chMDFe.ToString())[0].InnerText;
                        var nSeqEvento = Convert.ToInt32("0" + ((XmlElement)retConsSitNode1).GetElementsByTagName(TpcnResources.nSeqEvento.ToString())[0].InnerText);
                        var tpEvento = Convert.ToInt32("0" + ((XmlElement)retConsSitNode1).GetElementsByTagName(TpcnResources.tpEvento.ToString())[0].InnerText);
                        var dhRegEvento = Functions.GetDateTime/*Convert.ToDateTime*/(((XmlElement)retConsSitNode1).GetElementsByTagName(TpcnResources.dhRegEvento.ToString())[0].InnerText);
                        var versao = ((XmlElement)retConsSitNode1).Attributes[TpcnResources.versao.ToString()].InnerText;
                        var Idd = (((XmlElement)retConsSitNode1).GetElementsByTagName("infEvento")[0]).Attributes.GetNamedItem(TpcnResources.Id.ToString()).Value;

                        XmlDistEventoMDFe(emp, chNFe, nSeqEvento.ToString((Idd.Length <= 54 ? "00" : "000")), tpEvento, retConsSitNode1.OuterXml, string.Empty, dhRegEvento, false, versao);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// XML distribuição de evento da NF3e
        /// </summary>
        ...
        public void XmlDistEventoNF3e(int emp, string strXmlRetorno)
        {
            var docEventos = new XmlDocument();
            docEventos.Load(Functions.StringXmlToStreamUTF8(strXmlRetorno));
            var retProcEventoNF3eList = docEventos.GetElementsByTagName("procEventoNF3e");

            if (retProcEventoNF3eList != null)
            {
                foreach (XmlNode retConsSitNode in retProcEventoNF3eList)
                {
                    var cStat = ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.cStat.ToString())[0].InnerText;

                    if (cStat == "134" || cStat == "135" || cStat == "136")
                    {
                        var chNF3e = ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.chNF3e.ToString())[0].InnerText;
                        ...
                        XmlDistEventoMDFe(emp, chNF3e, nSeqEvento.ToString((idRetornado.Length <= 54 ? "00" : "000")), tpEvento, retConsSitNode.OuterXml, string.Empty, dhRegEvento, false, versao);
                    }
                }
            }
        }

        #endregion XmlDistEventoNF3e()

        #region XmlDistEventoNF3e()

        /// <summary>
        /// XML de distribuição do evento da NF3e
        /// </summary>
        /// <param name="emp">ID da empresa que vai ser trabalhado</param>
        ...
        public void XmlDistEventoNF3e(int emp, string chaveNF3e, string nSeqEvento, int tpEvento, string xmlEventoEnvio, string xmlRetornado, DateTime dhRegEvento, bool FromTaskEventos, string versao)
        {
            ...
            if (!FromTaskEventos && nf3eDeTerceiros)
            {
            ...
            if (xmlEventoEnvio.IndexOf("<procEventoNF3e>") >= 0)
            {
                protEnvioEvento = xmlEventoEnvio;
            }
            else
            {
                protEnvioEvento = "<procEventoNF3e versao=\"" + versao + "\" xmlns=\"" + NFeStrConstants.NAME_SPACE_NF3E + "\">" +
                           xmlEventoEnvio +
        ...
        /// <summary>
        /// XML distribuição de evento da NFCom
        /// </summary>
        /// <param name="emp">ID da empresa que vai ser trabalhado</param>
        /// <param name="strXmlRetorno">Retorno da SEFAZ no formato string</param>
        public void XmlDistEventoNFCom(int emp, string strXmlRetorno)
        {
            var docEventos = new XmlDocument();
            docEventos.Load(Functions.StringXmlToStreamUTF8(strXmlRetorno));
            var retProcEventoNFComList = docEventos.GetElementsByTagName("procEventoNFCom");

            if (retProcEventoNFComList != null)
            {
                foreach (XmlNode retConsSitNode in retProcEventoNFComList)
                {
                    var cStat = ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.cStat.ToString())[0].InnerText;

                    if (cStat == "134" || cStat == "135" || cStat == "136")
                    {
                        var chNFCom = ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.chNFCom.ToString())[0].InnerText;
                        var nSeqEvento = Convert.ToInt32("0" + ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.nSeqEvento.ToString())[0].InnerText);
                        var tpEvento = Convert.ToInt32("0" + ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.tpEvento.ToString())[0].InnerText);
                        var dhRegEvento = Functions.GetDateTime/*Convert.ToDateTime*/(((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.dhRegEvento.ToString())[0].InnerText);
                        var versao = ((XmlElement)retConsSitNode).Attributes[TpcnResources.versao.ToString()].InnerText;
                        ...
                        XmlDistEventoMDFe(emp, chNFCom, nSeqEvento.ToString((idRetornado.Length <= 54 ? "00" : "000")), tpEvento, retConsSitNode.OuterXml, string.Empty, dhRegEvento, false, versao);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// XML de distribuição do evento da NFCom
        /// </summary>
        /// <param name="emp">ID da empresa que vai ser trabalhado</param>
        ...
        public void XmlDistEventoNFCom(int emp, string chaveNFCom, string nSeqEvento, int tpEvento, string xmlEventoEnvio, string xmlRetornado, DateTime dhRegEvento, bool FromTaskEventos, string versao)
        {
            ...
            if (xmlEventoEnvio.IndexOf("<procEventoNFCom>") >= 0)
            {
                protEnvioEvento = xmlEventoEnvio;
            }
            else
            {
                protEnvioEvento = "<procEventoNFCom versao=\"" + versao + "\" xmlns=\"" + NFeStrConstants.NAME_SPACE_NFCOM + "\">" +
                           xmlEventoEnvio +
                           xmlRetornado.Replace("<?xml version=\"1.0\" encoding=\"utf-8\"?>", "") +
                           "</procEventoNFCom>";
            }
        ...
        #region XmlDistEventoBPe()

        /// <summary>
        /// XML distribuição de evento do BPe
        /// </summary>
        /// <param name="emp">ID da empresa que vai ser trabalhado</param>
        /// <param name="strXmlRetorno">Retorno da SEFAZ no formato string</param>
        public void XmlDistEventoBPe(int emp, string strXmlRetorno)
        {
            var docEventos = new XmlDocument();
            docEventos.Load(Functions.StringXmlToStreamUTF8(strXmlRetorno));
            var retProcEventoBPeList = docEventos.GetElementsByTagName("procEventoBPe");

            if (retProcEventoBPeList != null)
            {
                foreach (XmlNode retConsSitNode in retProcEventoBPeList)
                {
                    var cStat = ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.cStat.ToString())[0].InnerText;

                    if (cStat == "134" || cStat == "135" || cStat == "136")
                    {
                        var chBPe = ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.chBPe.ToString())[0].InnerText;
                        var nSeqEvento = Convert.ToInt32("0" + ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.nSeqEvento.ToString())[0].InnerText);
                        var tpEvento = Convert.ToInt32("0" + ((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.tpEvento.ToString())[0].InnerText);
                        var dhRegEvento = Functions.GetDateTime(((XmlElement)retConsSitNode).GetElementsByTagName(TpcnResources.dhRegEvento.ToString())[0].InnerText);
                        var versao = ((XmlElement)retConsSitNode).Attributes[TpcnResources.versao.ToString()].InnerText;
                        ...
                        XmlDistEventoBPe(emp, chBPe, nSeqEvento.ToString((idRetornado.Length <= 54 ? "00" : "000")), tpEvento, retConsSitNode.OuterXml, string.Empty, dhRegEvento, false, versao);
                    }
                }
            }
        }

        #endregion XmlDistEventoBPe()

        #region XmlDistEventoBPe()

        /// <summary>
        /// XML de distribuição do evento do BPe
        /// </summary>
        /// <param name="emp">ID da empresa que vai ser trabalhado</param>
        /// <param name="chaveBPe">Chave do BPe</param>
        /// <param name="nSeqEvento">Número de sequência do evento</param>
        /// <param name="tpEvento">Tipo de evento</param>
        /// <param name="xmlEventoEnvio">String do XML de evento enviado</param>
        /// <param name="xmlRetornado">String do XML retornado pela SEFAZ</param>
        /// <param name="dhRegEvento">Data e hora do registro do evento</param>
        /// <param name="FromTaskEventos">Indica se veio da task de eventos do BPe</param>
        /// <param name="versao">Versão do evento</param>
        public void XmlDistEventoBPe(int emp, string chaveBPe, string nSeqEvento, int tpEvento, string xmlEventoEnvio, string xmlRetornado, DateTime dhRegEvento, bool FromTaskEventos, string versao)
        {
            var tempXmlFile = PastaEnviados.Autorizados.ToString() + "\\" +
                Empresas.Configuracoes[emp].DiretorioSalvarComo.ToString(dhRegEvento) +
                chaveBPe + "_" + tpEvento.ToString() + "_" + nSeqEvento + Propriedade.ExtRetorno.ProcEventoBPe;

            var bpeDeTerceiros = !Functions.ChaveDFePertenceEmpresa(chaveBPe, Empresas.Configuracoes[emp].CNPJ, Empresas.Configuracoes[emp].UnidadeFederativaCodigo);
            ...
            if (!FromTaskEventos && bpeDeTerceiros)
            {
                if (!Empresas.Configuracoes[emp].GravarEventosDeTerceiros || string.IsNullOrEmpty(Empresas.Configuracoes[emp].PastaDownloadNFeDest))
                {
                    return;
            ...
            if (xmlEventoEnvio.IndexOf("<procEventoBPe>") >= 0)
            {
                protEnvioEvento = xmlEventoEnvio;
            }
            else
            {
                protEnvioEvento = "<procEventoBPe versao=\"" + versao + "\" xmlns=\"" + NFeStrConstants.NAME_SPACE_BPE + "\">" +
                           xmlEventoEnvio +
                           xmlRetornado.Replace("<?xml version=\"1.0\" encoding=\"utf-8\"?>", "") +
                           "</procEventoBPe>";
            }
        ...
        public string OndeNFeEstaGravada(int emp, string ChaveNFe, string extensao)
        {
            ...
            var ultimoDiaDoMes = DateTime.DaysInMonth(dte.Year, dte.Month);
            dte = new DateTime(dte.Year, dte.Month, ultimoDiaDoMes);
