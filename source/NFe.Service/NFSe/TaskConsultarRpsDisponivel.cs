...
{
    public class TaskConsultarRpsDisponivel : TaskAbst
    {
        public TaskConsultarRpsDisponivel(string arquivo)
        {
            Servico = Servicos.NFSeConsultarRpsDisponivel;
        ...
        public override void Execute()
        {
            ...
            try
            {
                Functions.DeletarArquivo(Empresas.Configuracoes[emp].PastaXmlRetorno + "\\" +
                    Functions.ExtrairNomeArq(NomeArquivoXML, Propriedade.Extensao(Propriedade.TipoEnvio.PedConsRpsDisp).EnvioXML) + Propriedade.ExtRetorno.ConsRpsDisp_ERR);
            ...
            catch (Exception ex)
            {
                try
                {
                    ...
                    TFunctions.GravarArqErroServico(NomeArquivoXML, Propriedade.Extensao(Propriedade.TipoEnvio.PedConsRpsDisp).EnvioXML, Propriedade.ExtRetorno.ConsRpsDisp_ERR, ex);
                }
                catch
                {
        ...
        private void ExecuteDLL(int emp, int municipio, PadraoNFSe padraoNFSe)
        {
            ...
            var finalArqEnvio = Propriedade.Extensao(Propriedade.TipoEnvio.PedConsRpsDisp).EnvioXML;
            var finalArqRetorno = Propriedade.Extensao(Propriedade.TipoEnvio.PedConsRpsDisp).RetornoXML;
            var versaoXML = DefinirVersaoXML(municipio, conteudoXML, padraoNFSe);
            var servico = Unimake.Business.DFe.Servicos.Servico.NFSeConsultarRpsDisponivel;
            ...
            var consultarRpsDisponivel = new Unimake.Business.DFe.Servicos.NFSe.ConsultarRpsDisponivel(conteudoXML, configuracao);
            consultarRpsDisponivel.Executar();

            vStrXmlRetorno = consultarRpsDisponivel.RetornoWSString;

            consultarRpsDisponivel.Dispose();

            XmlRetorno(finalArqEnvio, finalArqRetorno);

            /// grava o arquivo no FTP
            var filenameFTP = Path.Combine(Empresas.Configuracoes[emp].PastaXmlRetorno,
                Functions.ExtrairNomeArq(NomeArquivoXML, Propriedade.Extensao(Propriedade.TipoEnvio.PedConsRpsDisp).EnvioXML) + Propriedade.Extensao(Propriedade.TipoEnvio.PedConsRpsDisp).RetornoXML);
        ...
        private string DefinirVersaoXML(int codMunicipio, XmlDocument xmlDoc, PadraoNFSe padraoNFSe)
        {
            ...
            switch (padraoNFSe)
            {
                ...
                default:
                    throw new Exception("Padrão de NFSe " + padraoNFSe.ToString() + " não é válido para Consulta de RPS disponível.");
            }
