    ...
    public class TaskInutilizarNfse : TaskAbst
    {
        ...
        public override void Execute()
        {
            ...
            try
            {
                ...
                var padraoNFSe = Functions.BuscaPadraoNFSe(oDadosInuNfse.cMunicipio);

                ExecuteDLL(emp, oDadosInuNfse.cMunicipio, padraoNFSe);
            }
            catch
            {
                try
        ...
        private void ExecuteDLL(int emp, int municipio, PadraoNFSe padraoNFSe)
        {
            ...
            var finalArqEnvio = Propriedade.Extensao(Propriedade.TipoEnvio.PedInuNFSe).EnvioXML;
            var finalArqRetorno = Propriedade.Extensao(Propriedade.TipoEnvio.PedInuNFSe).RetornoXML;
            ...
            var configuracao = new Unimake.Business.DFe.Servicos.Configuracao
            {
                ...
                Servico = Unimake.Business.DFe.Servicos.Servico.NFSeSolicitacaoInutilizacao,
            ...
            var inutilizarNFSe = new Unimake.Business.DFe.Servicos.NFSe.SolicitacaoInutilizacao(conteudoXML, configuracao);
            inutilizarNFSe.Executar();

            vStrXmlRetorno = inutilizarNFSe.RetornoWSString;

            XmlRetorno(finalArqEnvio, finalArqRetorno);

            var filenameFTP = Path.Combine(Empresas.Configuracoes[emp].PastaXmlRetorno,
                Functions.ExtrairNomeArq(NomeArquivoXML, Propriedade.Extensao(Propriedade.TipoEnvio.PedInuNFSe).EnvioXML) + Propriedade.Extensao(Propriedade.TipoEnvio.PedInuNFSe).RetornoXML);
            if (File.Exists(filenameFTP))
            {
                new GerarXML(emp).XmlParaFTP(emp, filenameFTP);
            }

            inutilizarNFSe.Dispose();
        }
        ...
        private string DefinirVersaoXML(int codMunicipio, XmlDocument xmlDoc, PadraoNFSe padraoNFSe)
        {
            ...
            switch (padraoNFSe)
            {
                ...
                default:
                    throw new Exception("Padrão de NFSe " + padraoNFSe.ToString() + " não é válido para Inutilização de NFS-e.");
            }
