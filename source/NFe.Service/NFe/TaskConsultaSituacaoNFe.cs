                                    ...
                                    case "150": //NFe Autorizada fora do prazo
                                        ...
                                        if (File.Exists(strArquivoNFe))
                                        {
                                            ...
                                            if (Empresas.Configuracoes[emp].CompararDigestValueDFeRetornadoSEFAZ)
                                            {
                                                ...
                                                {
                                                    throw new Exception("O XML retornado na consulta de situação não contém a tag <digVal>. Isso indica que a SEFAZ não está atendendo à especificação do manual técnico nacional da NFe/NFCe. Sem essa tag, não é possível comparar o valor da tag <DigestValue> presente na assinatura do XML da NFe/NFCe, comprometendo a validação de que o protocolo retornado pertence à mesma nota fiscal. Por favor, entre em contato com a SEFAZ para relatar o problema, informando que a tag <digVal> está ausente no retorno da consulta de situação, o que está em desacordo com o padrão nacional.");
                                                }
                                            }
