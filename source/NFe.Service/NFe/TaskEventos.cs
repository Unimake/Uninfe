...
namespace NFe.Service
{
    public class TaskNFeEventos : TaskAbst
    {
        ...
        private readonly DadosenvEvento dadosEnvEvento;
            ...
            try
            {
                ...
                switch (EnumHelper.StringToEnum<ConvertTxt.tpEventos>(currentEvento))
                {
                    ...
                    case ConvertTxt.tpEventos.tpEvConciliacaoFinanceiraNFe:
                        Servico = Servicos.EventoConciliacaoFinanceiraNFe;
                        break;

                    case ConvertTxt.tpEventos.tpEvCancelamentoConciliacaoFinanceiraNFe:
                        Servico = Servicos.EventoCancelamentoConciliacaoFinanceiraNFe;
                        break;
                ...
                if (vXmlNfeDadosMsgEhXML)
                {
                    ...
                    var epecNFCe = (ehNFCe && Servico == Servicos.EventoEPEC && xml.Evento[0].InfEvento.COrgao == UFBrasil.SP); //EPEC NFCe da SEFAZ SP
                    ...
                    if (ehNFCe)
                    {
                        if (epecNFCe)
                        {
                            var recepcaoEvento = new Unimake.Business.DFe.Servicos.NFCe.RecepcaoEventoEPEC(xml, configuracao);
            ...
            if (Path.GetExtension(NomeArquivoXML).ToLower() == ".txt")
            {
                ...
                foreach (var cTexto in cLinhas)
                {
                    ...
                    switch (dados[0].ToLower())
                    {
                        ...
                        case "veraplic":
                            this.dadosEnvEvento.eventos[this.dadosEnvEvento.eventos.Count - 1].verAplic = dados[1].Trim();
                            break;
                        ...
                        case "detpag":
                            this.dadosEnvEvento.eventos[this.dadosEnvEvento.eventos.Count - 1].detPag.Add(new EventoConciliacaoFinanceira());
                            break;

                        case "detpag.indpag":
                            var ultimoIndPag = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoIndPag.detPag[ContarDetPag(ultimoIndPag)].IndPag = dados[1].Trim();
                            break;

                        case "detpag.tpag":
                            var ultimoTPag = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoTPag.detPag[ContarDetPag(ultimoTPag)].TPag = dados[1].Trim();
                            break;

                        case "detpag.xpag":
                            var ultimoXPag = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoXPag.detPag[ContarDetPag(ultimoXPag)].XPag = dados[1].Trim();
                            break;

                        case "detpag.vpag":
                            var ultimoVPag = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoVPag.detPag[ContarDetPag(ultimoVPag)].VPag = Convert.ToDouble("0" + dados[1].Trim().Replace(".", System.Globalization.NumberFormatInfo.CurrentInfo.NumberDecimalSeparator));
                            break;

                        case "detpag.dpag":
                            var ultimoDPag = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoDPag.detPag[ContarDetPag(ultimoDPag)].DPag = dados[1].Trim();
                            break;

                        case "detpag.cnpjpag":
                            var ultimoCnpjPag = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoCnpjPag.detPag[ContarDetPag(ultimoCnpjPag)].CNPJPag = dados[1].Trim();
                            break;

                        case "detpag.ufpag":
                            var ultimoUfPag = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoUfPag.detPag[ContarDetPag(ultimoUfPag)].UFPag = dados[1].Trim();
                            break;

                        case "detpag.cnpjif":
                            var ultimoCnpjIf = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoCnpjIf.detPag[ContarDetPag(ultimoCnpjIf)].CNPJIF = dados[1].Trim();
                            break;

                        case "detpag.tband":
                            var ultimoTBand = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoTBand.detPag[ContarDetPag(ultimoTBand)].TBand = dados[1].Trim();
                            break;

                        case "detpag.caut":
                            var ultimoCAut = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoCAut.detPag[ContarDetPag(ultimoCAut)].CAut = dados[1].Trim();
                            break;

                        case "detpag.cnpjreceb":
                            var ultimoCnpjReceb = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoCnpjReceb.detPag[ContarDetPag(ultimoCnpjReceb)].CNPJReceb = dados[1].Trim();
                            break;

                        case "detpag.ufreceb":
                            var ultimoUfReceb = this.dadosEnvEvento.eventos.LastOrDefault();
                            ultimoUfReceb.detPag[ContarDetPag(ultimoUfReceb)].UFReceb = dados[1].Trim();
                            break;

                        #endregion Conciliação Financeira

                        #region Cancelamento Conciliação financeira

                        case "cancconciliacaofinanceira.nprotevento":
                            var ultimoCancConciliacao = this.dadosEnvEvento.eventos.LastOrDefault();

                            ultimoCancConciliacao.cancelamentoConciliacaoFinanceira = new EventoCancelamentoConciliacaoFinanceira();
                            ultimoCancConciliacao.cancelamentoConciliacaoFinanceira.NProtEvento = dados[1].Trim();
                            break;

                        #endregion Cancelamento Conciliação financeira
                    }
                }
                foreach (var evento in this.dadosEnvEvento.eventos)
                {
                    ...
                    switch (tpe)
                    {
                        ...
                        case ConvertTxt.tpEventos.tpEvCancelamentoConciliacaoFinanceiraNFe:
                            evento.descEvento = "Cancelamento Conciliação Financeira";
                            break;
                    }
                    if (string.IsNullOrEmpty(evento.descEvento))
                    {
                    ...
                    if (evento.tpEmis == 0 && (tpe != ConvertTxt.tpEventos.tpEvConciliacaoFinanceiraNFe && tpe != ConvertTxt.tpEventos.tpEvCancelamentoConciliacaoFinanceiraNFe))
                    {
        ...
        /// <summary>
        /// Método que retorna a última posição do grupo detPag
        /// </summary>
        /// <param name="ultimoDetPag"></param>
        private int ContarDetPag(Evento ultimoDetPag)
        {
            if (ultimoDetPag.detPag.Count == 0)
            {
                throw new Exception("Não foi localizado a tag detPag, verifique se foi informado detPag|N no TXT");
            }

            return ultimoDetPag.detPag.Count - 1;
        }
                            ...
                            for (var i = 0; i < envEventosList.Count; ++i)
                            {
                                ...
                                else
                                {
                                    if (Empresas.Configuracoes[emp].DocumentosRejeitados)
                                    {
                                        ...
                                        sendMessageToWhatsApp.AlertNotification("Rejeição: " + Convert.ToInt32(cStatCons).ToString("000") + "-" + xMotivo.Trim(), "UNINFE - Evento NFe/NFCe rejeitado.");
                                    }
                                }
                            }
                        }
