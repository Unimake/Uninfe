...
namespace NFe.Service
{
    ...
    public class LerXML
    {
        ...
        /// <summary>
        /// Faz a leitura do XML da nota fiscal de energia eletrônica e disponibiliza os valores de algumas tag´
        /// </summary>
        /// <param name="xmlDocument">Conteúdo do XML para ser lido</param>
        public void NF3e(XmlDocument xmlDocument)
        {
            ClearDados();

            XmlNodeList infNF3eList = xmlDocument.GetElementsByTagName("infNF3e");

            foreach (XmlNode infNF3eNode in infNF3eList)
            {
                XmlElement infNF3eElemento = (XmlElement)infNF3eNode;

                if (infNF3eElemento.HasAttributes)
                {
                    oDadosNfe.chavenfe = infNF3eElemento.Attributes[TpcnResources.Id.ToString()].InnerText;
                    oDadosNfe.versao = infNF3eElemento.Attributes[TpcnResources.versao.ToString()].InnerText;
                }
        ...
        /// </summary>
        /// <param name="xmlDocument">Conteúdo do XML para ser lido</param>
        public void NFCom(XmlDocument xmlDocument)
        {
            ClearDados();

            XmlNodeList infNFComList = xmlDocument.GetElementsByTagName("infNFCom");

            foreach (XmlNode infNFComNode in infNFComList)
            {
                XmlElement infNFComElemento = (XmlElement)infNFComNode;

                if (infNFComElemento.HasAttributes)
                {
                    oDadosNfe.chavenfe = infNFComElemento.Attributes[TpcnResources.Id.ToString()].InnerText;
                    oDadosNfe.versao = infNFComElemento.Attributes[TpcnResources.versao.ToString()].InnerText;
                }
