    ...
    public class TaskMDFeEventos : TaskAbst
    {
        ...
        private void LerRetornoEvento(int emp)
        {
            ...
            vStrXmlRetorno = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
                ...
                "<chMDFe>41231180568835000181580010000023981201012210</chMDFe>" +
            ...
            */
            ...
            foreach (XmlNode retConsSitNode in retEnvRetornoList)
            {
                ...
                for (var i = 0; i < envEventosList.Count; ++i)
                {
                    ...
                    if (eleRetorno.GetElementsByTagName(TpcnResources.xMotivo.ToString()).Count > 0)
                    {
                        xMotivo = eleRetorno.GetElementsByTagName(TpcnResources.xMotivo.ToString())[0].InnerText;
                    }
                    ...
                    if (cStatCons == "132" || cStatCons == "135" || cStatCons == "136")
                    {
                        ...
                        var Idd = (docEventoOriginal.GetElementsByTagName("infEvento")[0]).Attributes.GetNamedItem(TpcnResources.Id.ToString()).Value;
                        var Id = TpcnResources.ID.ToString() + tpEvento + chMDFe + nSeqEvento.ToString((Idd.Length <= 54 ? "00" : "000"));
                        ///
                        ...
                        foreach (XmlNode env in docEventoOriginal.GetElementsByTagName("infEvento"))
                        {
                            if (Idd == Id)
                            {
                                ...
                                oGerarXML.XmlDistEventoMDFe(emp, chMDFe, nSeqEvento.ToString((Idd.Length <= 54 ? "00" : "000")), Convert.ToInt32(tpEvento), env.ParentNode.OuterXml, retConsSitElemento.OuterXml, dhRegEvento, true, versao);
                    ...
                    else
                    {
                        if (Empresas.Configuracoes[emp].DocumentosRejeitados)
                        {
                            ...
                            sendMessageToWhatsApp.AlertNotification("Rejeição: " + Convert.ToInt32(cStatCons).ToString("000") + "-" + xMotivo.Trim(), "UNINFE - Evento do MDFe rejeitado");
                        }
                    }
                }
            }
