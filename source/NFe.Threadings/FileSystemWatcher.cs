...
namespace NFe.Threadings
{
    public class FileSystemWatcher : IDisposable    //<<<danasa 1-5-2011
    {
        ...
        private readonly List<string> Directorys = new List<string>();
        ...
        public void StartWatch(int empresa)
        {
            var t = new Thread(new ParameterizedThreadStart(ProcessFiles))
            {
                IsBackground = true
            };
            t.Start(empresa);
        ...
        private void ProcessFiles(object empresa)
        {
            var pastaGeral = false;
            var emp = 0;
            foreach (var item in Directorys)
            {
                var directory = item;
            ...
            //
            _pool = new System.Threading.Semaphore(20, 40);
                                    ...
                                    foreach (var fi in files)
                                    {
                                        ...
                                        if (File.Exists(fi.FullName))
                                        {
                                            if (!Functions.FileInUse(fi.FullName))
                                            {
                                                ...
                                                {
                                                    var tempoDesdeCriacao = DateTime.Now - fi.CreationTime;

                                                    if (tempoDesdeCriacao.TotalMinutes >= 1)
                                                    {
                                                        //Se o XML estiver parado a mais de 1 minuto na pasta temp, não será mais processado, será movido para a pasta de erro.
                                                        Functions.Move(arqTemp, Path.Combine(Empresas.Configuracoes[emp].PastaXmlErro, fi.Name));

                                                        Auxiliar.WriteLog($"O arquivo '{arqTemp}' permaneceu na pasta 'Temp' por mais de 1 minuto sem ser processado. Por isso, foi movido para a pasta de XMLs com erro.", false);
                                                    }
                                                    else
                                                    {
                                                        // Gravar log e aguardar para tentar novamente na próxima execução
                                                        Auxiliar.WriteLog($"O arquivo '{arqTemp}' já está em processamento. Aguarde a conclusão antes de tentar gerar um novo XML com o mesmo nome.", false);
                                                    }
                                                }
                                                else
                                                {
        ...
        {
            get => CancelProcess; set => CancelProcess = value;
        }
