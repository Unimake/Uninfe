...
namespace NFe.UI.Formularios
{
    public partial class userConfiguracao_resptecnico : MetroFramework.Controls.MetroUserControl
    {
        ...
        public void Validar(bool salvando = true)
        {
            ...
            // Define os campos obrigatórios do responsável técnico
            var camposObrigatorios = new Dictionary<string, string>
            {
                { "CNPJ", empresa.RespTecCNPJ },
                { "Contato", empresa.RespTecXContato },
                { "Email", empresa.RespTecEmail },
                { "Telefone", empresa.RespTecTelefone }
            };

            // Verifica se pelo menos um dos campos obrigatórios foi preenchido
            bool algumPreenchido = camposObrigatorios.Values.Any(valor => !string.IsNullOrWhiteSpace(valor));

            // Verifica se algum dos campos obrigatórios está vazio
            bool algumVazio = camposObrigatorios.Values.Any(valor => string.IsNullOrWhiteSpace(valor));

            // Impede a validação se houver campos obrigatórios preenchidos parcialmente
            if (algumPreenchido && algumVazio)
            {
                ...
                throw new Exception($"Os seguintes campos precisam ser preenchidos:{camposFaltando}");
            }
        }
