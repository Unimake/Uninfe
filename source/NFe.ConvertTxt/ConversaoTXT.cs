...
namespace NFe.ConvertTxt
{
    public class ConversaoTXT
    {
        ...
        public bool Converter(string cArquivo, string cFolderDestino)
        {
            ...
            if (!this.CarregarArquivo(cArquivo)) return false;
            ...
            foreach (List<string> conteudoNota in this.xConteudoArquivo.Values)
            {
                ...
                {
                    this.GerarXmlDaNota(cFolderDestino, cArquivo);
                }
        ...
        private void GerarXmlDaNota(string pastaDestino, string arquivoOrigem)
        {
            var geradorXml = new NFeW();
            try
            {
            ...
            {
                geradorXml.cMensagemErro += ex.Message;
            }

            this.cMensagemErro = geradorXml.cMensagemErro;
        }

        private void ExcluirArquivosGerados()
        {
            foreach (txtTOxmlClassRetorno arquivoConvertido in this.cRetorno)
            {
                if (File.Exists(arquivoConvertido.XMLFileName))
                {
                    new FileInfo(arquivoConvertido.XMLFileName).Delete();
                }
            }
        }
        /// <summary>
