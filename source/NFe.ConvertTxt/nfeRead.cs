...
namespace NFe.ConvertTxt
{
    public class nfeRead
    {
        ...
        private decimal readDecimal(object element, TpcnResources tag)
        {
            char charSeparator = System.Globalization.NumberFormatInfo.CurrentInfo.NumberDecimalSeparator[0];
            return Convert.ToDecimal("0" + this.readValue(element, tag).Replace(".", charSeparator.ToString()));
        }
                    ...
                    case "infnfe":
                        ...
                        foreach(XmlNode nodeinfNFe in nodeNFe.ChildNodes)
                        {
                            switch(nodeinfNFe.LocalName)
                            {
                                ...
                                case "agropecuario":
                                    processaAgropecuario(nodeinfNFe);
                                    break;
                            }
                        }
                        break;
        ...
        private void processaAgropecuario(XmlNode nodeAgropecuario)
        {
            var nodeDefensivo = ((XmlElement)nodeAgropecuario).GetElementsByTagName(TpcnResources.defensivo.ToString());
            var nodeGuiaTransito = ((XmlElement)nodeAgropecuario).GetElementsByTagName(TpcnResources.guiaTransito.ToString());

            if (nodeDefensivo.Count > 0)
            {
                foreach (XmlNode nodeDefensivoAgropecuario in nodeDefensivo)
                {
                    var defensivo = new Defensivo();
                    defensivo.nReceituario = nodeDefensivoAgropecuario.ChildNodes[0].InnerText;
                    defensivo.CPFResptec = nodeDefensivoAgropecuario.ChildNodes[1].InnerText;

                    nfe.agropecuario.defensivo.Add(defensivo);
                }
            }
            else if (nodeGuiaTransito.Count > 0)
            {
                foreach (XmlNode nodeGuiaTransitoAgropecuario in nodeGuiaTransito)
                {
                    nfe.agropecuario.guiaTransito.tpGuia = (TpcnTipoGuia)this.readInt32(nodeGuiaTransitoAgropecuario, TpcnResources.tpGuia);
                    nfe.agropecuario.guiaTransito.UFGuia = this.readValue(nodeGuiaTransitoAgropecuario, TpcnResources.UFGuia);
                    nfe.agropecuario.guiaTransito.serieGuia = this.readValue(nodeGuiaTransitoAgropecuario, TpcnResources.serieGuia);
                    nfe.agropecuario.guiaTransito.nGuia = this.readValue(nodeGuiaTransitoAgropecuario, TpcnResources.nGuia);
                }
            }            
        }
        ...
        private void processaTOTAL(XmlNode nodetotal)
        {
            ...
            foreach (XmlNode nodeISTot in ((XmlElement)nodetotal).GetElementsByTagName("ISTot"))
            {
                nfe.Total.ISTot.vIS = this.readDouble(nodeISTot, TpcnResources.vIS);
            }

            foreach (XmlNode nodeIBSCBSTot in ((XmlElement)nodetotal).GetElementsByTagName("IBSCBSTot"))
            {
                nfe.Total.IBSCBSTot.vBCIBSCBS = this.readDouble(nodeIBSCBSTot, TpcnResources.vBCIBSCBS);

                foreach (XmlNode nodeIBSCBSTotgIBS in ((XmlElement)nodeIBSCBSTot).GetElementsByTagName("gIBS"))
                {
                    foreach (XmlNode nodeIBSCBSTotgIBSgIBSUF in ((XmlElement)nodeIBSCBSTotgIBS).GetElementsByTagName("gIBSUF"))
                    {
                        nfe.Total.IBSCBSTot.gIBS.gIBSUF.vDif = this.readDouble(nodeIBSCBSTotgIBSgIBSUF, TpcnResources.vDif);
                        nfe.Total.IBSCBSTot.gIBS.gIBSUF.vDevTrib = this.readDouble(nodeIBSCBSTotgIBSgIBSUF, TpcnResources.vDevTrib);
                        nfe.Total.IBSCBSTot.gIBS.gIBSUF.vIBSUF = this.readDouble(nodeIBSCBSTotgIBSgIBSUF, TpcnResources.vIBSUF);
                    }

                    foreach (XmlNode nodeIBSCBSTotgIBSgIBSMun in ((XmlElement)nodeIBSCBSTotgIBS).GetElementsByTagName("gIBSMun"))
                    {
                        nfe.Total.IBSCBSTot.gIBS.gIBSMun.vDif = this.readDouble(nodeIBSCBSTotgIBSgIBSMun, TpcnResources.vDif);
                        nfe.Total.IBSCBSTot.gIBS.gIBSMun.vDevTrib = this.readDouble(nodeIBSCBSTotgIBSgIBSMun, TpcnResources.vDevTrib);
                        nfe.Total.IBSCBSTot.gIBS.gIBSMun.vIBSMun = this.readDouble(nodeIBSCBSTotgIBSgIBSMun, TpcnResources.vIBSMun);
                    }

                    nfe.Total.IBSCBSTot.gIBS.vIBS = this.readDouble(nodeIBSCBSTotgIBS, TpcnResources.vIBS);
                    nfe.Total.IBSCBSTot.gIBS.vCredPres = this.readDouble(nodeIBSCBSTotgIBS, TpcnResources.vCredPres);
                    nfe.Total.IBSCBSTot.gIBS.vCredPresCondSus = this.readDouble(nodeIBSCBSTotgIBS, TpcnResources.vCredPresCondSus);
                }

                foreach (XmlNode nodeIBSCBSTotgCBS in ((XmlElement)nodeIBSCBSTot).GetElementsByTagName("gCBS"))
                {
                    nfe.Total.IBSCBSTot.gCBS.vDif = this.readDouble(nodeIBSCBSTotgCBS, TpcnResources.vDif);
                    nfe.Total.IBSCBSTot.gCBS.vDevTrib = this.readDouble(nodeIBSCBSTotgCBS, TpcnResources.vDevTrib);
                    nfe.Total.IBSCBSTot.gCBS.vCBS = this.readDouble(nodeIBSCBSTotgCBS , TpcnResources.vCBS);
                    nfe.Total.IBSCBSTot.gCBS.vCredPres = this.readDouble(nodeIBSCBSTotgCBS, TpcnResources.vCredPres);
                    nfe.Total.IBSCBSTot.gCBS.vCredPresCondSus = this.readDouble(nodeIBSCBSTotgCBS, TpcnResources.vCredPresCondSus);
                }

                foreach (XmlNode nodeIBSCBSTotgMono in ((XmlElement)nodeIBSCBSTot).GetElementsByTagName("gMono"))
                {
                    nfe.Total.IBSCBSTot.gMono.vIBSMono = this.readDouble(nodeIBSCBSTotgMono, TpcnResources.vIBSMono);
                    nfe.Total.IBSCBSTot.gMono.vCBSMono = this.readDouble(nodeIBSCBSTotgMono, TpcnResources.vCBSMono);
                    nfe.Total.IBSCBSTot.gMono.vIBSMonoReten = this.readDouble(nodeIBSCBSTotgMono, TpcnResources.vIBSMonoReten);
                    nfe.Total.IBSCBSTot.gMono.vCBSMonoReten = this.readDouble(nodeIBSCBSTotgMono, TpcnResources.vCBSMonoReten);
                    nfe.Total.IBSCBSTot.gMono.vIBSMonoRet = this.readDouble(nodeIBSCBSTotgMono, TpcnResources.vIBSMonoRet);
                    nfe.Total.IBSCBSTot.gMono.vCBSMonoRet = this.readDouble(nodeIBSCBSTotgMono, TpcnResources.vCBSMonoRet);
                }
            }

            nfe.Total.vNFTot = this.readDouble(nodetotal, TpcnResources.vNFTot);
        }
        ...
        private void processaPROD(XmlNode nodedet)
        {
            ...
            detInfo.vItem = this.readDouble(nodedet, TpcnResources.vItem);
            ...
            foreach(XmlNode nodedetprod in ((XmlElement)nodedet).GetElementsByTagName("prod"))
            {
                ...
                detInfo.Prod.nItemPed = this.readValue(ele, TpcnResources.nItemPed);
                detInfo.Prod.qCom = this.readDecimal(ele, TpcnResources.qCom);
                detInfo.Prod.qTrib = this.readDecimal(ele, TpcnResources.qTrib);
                ...
                detInfo.Prod.vUnCom = this.readDecimal(ele, TpcnResources.vUnCom);
                detInfo.Prod.vUnTrib = this.readDecimal(ele, TpcnResources.vUnTrib);
            ...
            foreach (XmlNode nodedetImposto in ((XmlElement)nodedet).GetElementsByTagName("imposto"))
            {
                ...
                foreach (XmlNode nodeDetImpostoIS in ((XmlElement)nodedetImposto).GetElementsByTagName("IS"))
                {
                    detInfo.Imposto.IS.CSTIS = this.readValue(nodeDetImpostoIS, TpcnResources.CSTIS);
                ...
                #endregion --Imposto->IS

                #region --Imposto->IBSCBS

                foreach (XmlNode nodeDetImpostoIBSCBS in ((XmlElement)nodedetImposto).GetElementsByTagName("IBSCBS"))
                {
                    detInfo.Imposto.IBSCBS.CST = this.readValue(nodeDetImpostoIBSCBS, TpcnResources.CST);
                    detInfo.Imposto.IBSCBS.cClassTrib = this.readValue(nodeDetImpostoIBSCBS, TpcnResources.cClassTrib);

                    foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBS in ((XmlElement)nodeDetImpostoIBSCBS).GetElementsByTagName("gIBSCBS"))
                    {
                        detInfo.Imposto.IBSCBS.gIBSCBS.vBC = this.readDouble(nodeDetImpostoIBSCBSgIBSCBS, TpcnResources.vBC);

                        #region --Imposto->IBSCBS->gIBSCBS->gIBSUF

                        foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgIBSUF in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBS).GetElementsByTagName("gIBSUF"))
                        {
                            detInfo.Imposto.IBSCBS.gIBSCBS.gIBSUF.pIBSUF = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSUF, TpcnResources.pIBSUF);

                            foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgIBSUFgDif in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBSgIBSUF).GetElementsByTagName("gDif"))
                            {
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSUF.gDif.pDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSUFgDif, TpcnResources.pDif);
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSUF.gDif.vDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSUFgDif, TpcnResources.pDif);
                            }
                            ...
                            foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgIBSUFgRed in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBSgIBSUF).GetElementsByTagName("gRed"))
                            {
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSUF.gRed.pRedAliq = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSUFgRed, TpcnResources.pRedAliq);
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSUF.gRed.pAliqEfet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSUFgRed, TpcnResources.pAliqEfet);
                            }

                            detInfo.Imposto.IBSCBS.gIBSCBS.gIBSUF.vIBSUF = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSUF, TpcnResources.vIBSUF);
                        }

                        #endregion --Imposto->IBSCBS->gIBSCBS->gIBSUF

                        #region --Imposto->IBSCBS->gIBSCBS->gIBSMun

                        foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgIBSMun in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBS).GetElementsByTagName("gIBSMun"))
                        {
                            detInfo.Imposto.IBSCBS.gIBSCBS.gIBSMun.pIBSMun = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSMun, TpcnResources.pIBSMun);

                            foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgIBSMungDif in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBSgIBSMun).GetElementsByTagName("gDif"))
                            {
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSMun.gDif.pDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSMungDif, TpcnResources.pDif);
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSMun.gDif.vDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSMungDif, TpcnResources.pDif);
                            }
                            ...
                            foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgIBSMungRed in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBSgIBSMun).GetElementsByTagName("gRed"))
                            {
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSMun.gRed.pRedAliq = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSMungRed, TpcnResources.pRedAliq);
                                detInfo.Imposto.IBSCBS.gIBSCBS.gIBSMun.gRed.pAliqEfet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSMungRed, TpcnResources.pAliqEfet);
                            }

                            detInfo.Imposto.IBSCBS.gIBSCBS.gIBSMun.vIBSMun = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgIBSMun, TpcnResources.vIBSMun);
                        }

                        #endregion--Imposto->IBSCBS->gIBSCBS->gIBSMun

                        #region --Imposto->IBSCBS->gIBSCBS->gCBS

                        foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgCBS in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBS).GetElementsByTagName("gCBS"))
                        {
                            detInfo.Imposto.IBSCBS.gIBSCBS.gCBS.pCBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgCBS, TpcnResources.pCBS);

                            foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgCBSgDif in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBSgCBS).GetElementsByTagName("gDif"))
                            {
                                detInfo.Imposto.IBSCBS.gIBSCBS.gCBS.gDif.pDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgCBSgDif, TpcnResources.pDif);
                                detInfo.Imposto.IBSCBS.gIBSCBS.gCBS.gDif.vDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgCBSgDif, TpcnResources.pDif);
                            }
                            ...
                            foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgCBSgRed in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBSgCBS).GetElementsByTagName("gRed"))
                            {
                                detInfo.Imposto.IBSCBS.gIBSCBS.gCBS.gRed.pRedAliq = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgCBSgRed, TpcnResources.pRedAliq);
                                detInfo.Imposto.IBSCBS.gIBSCBS.gCBS.gRed.pAliqEfet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgCBSgRed, TpcnResources.pAliqEfet);
                            }

                            detInfo.Imposto.IBSCBS.gIBSCBS.gCBS.vCBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgCBS, TpcnResources.vCBS);
                        }

                        #endregion --Imposto->IBSCBS->gIBSCBS->gCBS

                        #region --Imposto->IBSCBS->gIBSCBS->gTribRegular

                        foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSgTribRegular in ((XmlElement)nodeDetImpostoIBSCBSgIBSCBS).GetElementsByTagName("gTribRegular"))
                        {
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.CSTReg = this.readValue(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.CSTReg);
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.cClassTribReg = this.readValue(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.cClassTribReg);
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.pAliqEfetRegIBSUF = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.pAliqEfetRegIBSUF);
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.vTribRegIBSUF = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.vTribRegIBSUF);
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.pAliqEfetRegIBSMun = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.pAliqEfetRegIBSMun);
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.vTribRegIBSMun = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.vTribRegIBSMun);
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.pAliqEfetRegCBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.pAliqEfetRegCBS);
                            detInfo.Imposto.IBSCBS.gIBSCBS.gTribRegular.vTribRegCBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSgTribRegular, TpcnResources.vTribRegCBS);
                        }

                        #endregion --Imposto->IBSCBS->gIBSCBS->gTribRegular
                    }

                    foreach (XmlNode nodeDetImpostoIBSCBSgIBSCBSMono in ((XmlElement)nodeDetImpostoIBSCBS).GetElementsByTagName("gIBSCBSMono"))
                    {
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoPadrao.qBCMono = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.qBCMono);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoPadrao.adRemIBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.adRemIBS);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoPadrao.adRemCBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.adRemCBS);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoPadrao.vIBSMono = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vIBSMono);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoPadrao.vCBSMono = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vCBSMono);

                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoReten.qBCMonoReten = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.qBCMonoReten);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoReten.adRemIBSReten = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.adRemIBSReten);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoReten.vIBSMonoReten = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vIBSMonoReten);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoReten.adRemCBSReten = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.adRemCBSReten);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoReten.vCBSMonoReten = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vCBSMonoReten);

                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoRet.qBCMonoRet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.qBCMonoRet);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoRet.adRemIBSRet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.adRemIBSRet);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoRet.vIBSMonoRet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vIBSMonoRet);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoRet.adRemCBSRet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.adRemCBSRet);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoRet.vCBSMonoRet = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vCBSMonoRet);

                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoDif.pDifIBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.pDifIBS);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoDif.vIBSMonoDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vIBSMonoDif);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoDif.pDifCBS = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.pDifCBS);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.gMonoDif.vCBSMonoDif = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vCBSMonoDif);

                        detInfo.Imposto.IBSCBS.gIBSCBSMono.vTotIBSMonoItem = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vTotIBSMonoItem);
                        detInfo.Imposto.IBSCBS.gIBSCBSMono.vTotCBSMonoItem = this.readDouble(nodeDetImpostoIBSCBSgIBSCBSMono, TpcnResources.vTotCBSMonoItem);
                    }
                }

                #endregion --Imposto->IBSCBS
            }

            #endregion --prod->imposto

            #region -- prod->DFeReferenciado

            foreach (XmlNode nodedetDfeReferenciado in ((XmlElement)nodedet).GetElementsByTagName("DFeReferenciado"))
            {
                detInfo.DfeReferenciado.chaveAcesso = this.readValue(nodedetDfeReferenciado, TpcnResources.chaveAcesso);
                detInfo.DfeReferenciado.nItem = this.readInt32(nodedetDfeReferenciado, TpcnResources.NItem);
            }

            #endregion -- prod->DFeReferenciado
        ...
        private void processaIDE(XmlNode nodeinfNFe)
        {
            ...
            nfe.ide.cMunFGIBS = this.readInt32(nodeinfNFe, TpcnResources.cMunFGIBS);
            ...
            nfe.ide.tpNFDebito = (TpcnTipoNFDebito)this.readInt32(nodeinfNFe, TpcnResources.tpNFDebito);
            nfe.ide.tpNFCredito = (TpcnTipoNFCredito)this.readInt32(nodeinfNFe, TpcnResources.tpNFCredito);
            ...
            foreach(XmlNode nodeFilho in nodeinfNFe.ChildNodes)
            {
                if(nodeFilho.LocalName.Equals("NFref"))
                {
                    foreach(XmlNode nodeNFrefItem in nodeFilho.ChildNodes)
                    {
                ...
                else if (nodeFilho.LocalName.Equals("gCompraGov"))
                {
                    XmlElement gCompraGov = nodeFilho as XmlElement;
