...
O serviço de retificação da operação de transporte do CIOT permite que o ERP envie uma solicitação para corrigir dados de uma operação de transporte já registrada. O ERP grava o XML de retificação na pasta de envio, o UniNFe assina o XML, transmite a solicitação e grava o retorno na pasta configurada para retornos.
...
- A operação de transporte que será retificada já existe no CIOT.
- O XML contém os dados da operação que precisam ser retificados, conforme o leiaute do CIOT.

## Arquivo de envio

O ERP deve gerar o XML de retificação na pasta de envio da empresa com o final fixo:

```text
<identificador>-ped-eve.xml
```

O `<identificador>` deve ser único para evitar conflito entre solicitações. Uma forma prática é usar uma composição com a operação e a palavra `retificacao`.

Exemplo de nome:

```text
retificacaoOperacaoTransporte-ped-eve.xml
```

O conteúdo do XML deve usar a estrutura de retificação de operação de transporte:
...
2. O UniNFe lê o XML `RetificacaoOperacaoTransporte`.
...
5. O UniNFe envia a retificação ao serviço CIOT.
6. O retorno do serviço é gravado na pasta de retorno como `<identificador>-ret-ped-eve.xml`.
7. Se o retorno indicar que a retificação foi aceita, o UniNFe grava o XML processado `<identificador>-procEventoCIOT.xml` em `Enviados\Autorizados`.
8. O XML assinado `<identificador>-ped-eve.xml` também é movido para `Enviados\Autorizados`.
9. Se a retificação for rejeitada, o XML assinado em processamento é movido para a pasta de erros e o ERP deve tratar a mensagem de retorno.
...
|---|---|---|---|
| Envio pelo ERP | Pasta de envio | `<identificador>-ped-eve.xml` | Arquivo criado pelo ERP para solicitar a retificação da operação de transporte. |
| Em processamento | `Enviados\EmProcessamento` | `<identificador>-ped-eve.xml` | XML assinado pelo UniNFe enquanto a retificação está sendo processada. |
...
| XML processado | `Enviados\Autorizados\<subpasta por data>` | `<identificador>-procEventoCIOT.xml` | Retificação aceita. É o XML principal para armazenamento do evento processado. |
...
| XML rejeitado | Pasta de erros configurada | `<identificador>-ped-eve.xml` | Retificação rejeitada ou não aceita pelo serviço CIOT. |
...
Esse arquivo contém a resposta do serviço CIOT. Quando a retificação for aceita, o ERP deve localizar e armazenar o XML processado:

```text
<identificador>-procEventoCIOT.xml
```

O XML processado é gravado em `Enviados\Autorizados`, dentro da subpasta criada conforme a data da retificação retornada pelo serviço e a configuração de organização dos XMLs enviados.
...
- XML fora da estrutura esperada para `RetificacaoOperacaoTransporte`.
