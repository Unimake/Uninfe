# Consulta status de serviço da DCe

A consulta status de serviço da DCe permite que o ERP verifique se o webservice de DCe da SEFAZ está disponível para determinada UF e ambiente. Ela deve ser usada antes de operações fiscais ou sempre que houver suspeita de indisponibilidade no serviço.

Esta consulta não envia DCe, não autoriza documento e não gera XML de distribuição. O resultado serve para orientar o ERP, o suporte e o usuário sobre a disponibilidade do webservice.

## Quando usar

Use este serviço quando:

- O ERP precisa confirmar se o webservice de DCe está operando.
- O envio de DCe falhou por suspeita de indisponibilidade da SEFAZ.
...
Para consultar a situação de uma DCe específica pela chave de acesso, use a [consulta de situação da DCe](consulta-situacao.md).
...
```text
consStatServDCe-ped-sta.xml
```

O conteúdo do XML deve usar a estrutura de consulta status de serviço da DCe:

```xml
<consStatServDCe versao="1.00" xmlns="http://www.portalfiscal.inf.br/dce">
  <tpAmb>2</tpAmb>
  <cUF>35</cUF>
  <xServ>STATUS</xServ>
</consStatServDCe>
...
|---|---|
| `versao` | Versão do leiaute da consulta de status da DCe. |
...
2. O UniNFe lê o XML e identifica a consulta status de serviço da DCe.
...
|---|---|---|---|
| Pedido de consulta | Pasta de envio | `<identificador>-ped-sta.xml` | Arquivo criado pelo ERP para consultar a disponibilidade do webservice de DCe. |
...
Esse arquivo contém a resposta da SEFAZ para a UF e o ambiente consultados. O ERP deve analisar o status e o motivo retornados para decidir se pode iniciar ou continuar as operações de DCe.
