...
O serviço de eventos da NFGas permite que o ERP envie eventos vinculados a uma NFGas já emitida. O ERP grava o XML do evento na pasta de envio, o UniNFe assina o XML, transmite o evento para a SEFAZ e grava o retorno na pasta de retorno.

Use este serviço quando for necessário registrar uma ocorrência fiscal relacionada à NFGas, como o cancelamento do documento.

## Eventos atendidos nos exemplos

O exemplo disponível para NFGas cobre este tipo de evento:

| Tipo de evento | Descrição no XML |
|---|---|
| `110111` | Cancelamento |

Use o tipo de evento, o detalhamento e as regras fiscais conforme o manual da NFGas e conforme a situação real do documento.
...
- A NFGas referenciada no evento existe e a chave informada é a chave correta.
- O ambiente do evento é o mesmo ambiente em que a NFGas foi emitida.
...
O `<identificador>` deve ser único para o evento. Uma forma prática é usar uma composição com o tipo de evento, a chave da NFGas e a sequência.

Exemplo:

```text
1101113526051234567800019576001000000010000000100001-ped-eve.xml
```

O conteúdo do XML deve usar a estrutura de evento da NFGas:

```xml
<?xml version="1.0" encoding="utf-8"?>
<eventoNFGas versao="1.00" xmlns="http://www.portalfiscal.inf.br/nfgas">
  <infEvento Id="ID1101113526051234567800019576001000000010000000100001">
    <cOrgao>35</cOrgao>
    <tpAmb>1</tpAmb>
    <CNPJ>12345678000195</CNPJ>
    <chNFGas>35260512345678000195760010000000100000001000</chNFGas>
    <dhEvento>2026-05-18T10:00:00-03:00</dhEvento>
...
|---|---|
| `infEvento/@Id` | Identificador do evento. Deve ser compatível com o tipo de evento, chave da NFGas e sequência. |
| `cOrgao` | Código da UF ou órgão responsável pelo evento. |
| `tpAmb` | Ambiente do evento. Use o mesmo ambiente da NFGas. |
| `CNPJ` | CNPJ do emissor do evento. |
| `chNFGas` | Chave de acesso da NFGas vinculada ao evento. |
...
| `nProt` | Número do protocolo da NFGas, quando exigido pelo tipo de evento. |
...
2. O UniNFe lê o XML `eventoNFGas`.
...
|---|---|---|---|
| Pedido do evento | Pasta de envio | `<identificador>-ped-eve.xml` | Arquivo criado pelo ERP para enviar o evento da NFGas. |
...
| Evento processado | `Enviados\Autorizados\<subpasta por data>` | `<chaveNFGas>_<tipoEvento>_<sequencia>-procEventoNFGas.xml` | Evento aceito pela SEFAZ. O conteúdo do arquivo é um XML `procEventoNFGas`. |
| Backup do evento processado | Pasta de backup, quando configurada | `<chaveNFGas>_<tipoEvento>_<sequencia>-procEventoNFGas.xml` | Cópia de segurança do evento aceito. |
...
Quando o evento for aceito, o UniNFe gera um XML processado do evento com o conteúdo `procEventoNFGas`. O arquivo é gravado em `Enviados\Autorizados`, dentro da subpasta de data configurada, usando o padrão:

```text
<chaveNFGas>_<tipoEvento>_<sequencia>-procEventoNFGas.xml
```
...
- Ambiente do evento diferente do ambiente da NFGas.
- Falha de assinatura.
...
- Use sempre o final `-ped-eve.xml` para envio de evento da NFGas.
...
- Use o mesmo ambiente da NFGas original.
