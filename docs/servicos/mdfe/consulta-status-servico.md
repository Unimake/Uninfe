# Consulta status de serviço do MDFe

A consulta status de serviço do MDFe permite que o ERP verifique se o webservice de MDFe da SEFAZ está disponível para determinada UF e ambiente. Ela deve ser usada antes de operações fiscais ou sempre que houver suspeita de indisponibilidade no serviço.

Esta consulta não envia MDFe, não autoriza documento, não encerra manifesto e não gera XML de distribuição. O resultado serve para orientar o ERP, o suporte e o usuário sobre a disponibilidade do webservice.

## Quando usar

Use este serviço quando:

- O ERP precisa confirmar se o webservice de MDFe está operando.
- O envio, encerramento, cancelamento ou evento de MDFe falhou por suspeita de indisponibilidade da SEFAZ.
- O suporte precisa validar ambiente, UF, certificado, proxy ou comunicação.
- A empresa deseja testar a comunicação antes de iniciar operações com MDFe.

Para consultar a situação de um MDFe específico pela chave de acesso, use a [consulta de situação do MDFe](consulta-situacao.md).
...
```text
20170210T100210-ped-sta.xml
```

O conteúdo do XML deve usar a estrutura de consulta status de serviço do MDFe:
...
|---|---|
| `versao` | Versão do leiaute da consulta de status do MDFe. |
...
2. O UniNFe lê o XML e identifica a consulta status de serviço do MDFe.
3. O UniNFe aplica as configurações da empresa, certificado, UF, proxy e conexão TLS quando configurado.
...
flowchart TD
    ...
    B --> C["UniNFe lê consStatServMDFe"]
    C --> D["Aplica certificado, UF, proxy e TLS"]
    D --> E["Consulta status do serviço de MDFe na SEFAZ"]
...
|---|---|---|---|
| Pedido de consulta | Pasta de envio | `<identificador>-ped-sta.xml` | Arquivo criado pelo ERP para consultar a disponibilidade do webservice de MDFe. |
...
Esse arquivo contém a resposta da SEFAZ para a UF e o ambiente consultados. O ERP deve analisar o status e o motivo retornados para decidir se pode iniciar ou continuar as operações de MDFe.
...
- Ambiente ou UF ausente ou inválido.
...
- Não trate esta consulta como autorização, encerramento ou cancelamento de MDFe; ela apenas informa a disponibilidade do webservice.
