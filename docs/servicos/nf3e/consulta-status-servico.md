# Consulta status de serviço da NF3e

A consulta status de serviço da NF3e permite que o ERP verifique se o webservice de NF3e da SEFAZ está disponível para determinada UF e ambiente. Ela deve ser usada antes de operações fiscais ou sempre que houver suspeita de indisponibilidade no serviço.

Esta consulta não envia NF3e, não autoriza documento e não gera XML de distribuição. O resultado serve para orientar o ERP, o suporte e o usuário sobre a disponibilidade do webservice.

## Quando usar

Use este serviço quando:

- O ERP precisa confirmar se o webservice de NF3e está operando.
- O envio de NF3e falhou por suspeita de indisponibilidade da SEFAZ.
...
Para consultar a situação de uma NF3e específica pela chave de acesso, use a [consulta de situação da NF3e](consulta-situacao.md).
...
```text
consStatServ-ped-sta.xml
```

O conteúdo do XML deve usar a estrutura de consulta status de serviço da NF3e:

```xml
<?xml version="1.0" encoding="utf-8"?>
<consStatServNF3e versao="1.00" xmlns="http://www.portalfiscal.inf.br/nf3e">
  <tpAmb>1</tpAmb>
  <cUF>41</cUF>
  <xServ>STATUS</xServ>
</consStatServNF3e>
...
|---|---|
| `versao` | Versão do leiaute da consulta de status da NF3e. |
...
2. O UniNFe lê o XML e identifica a consulta status de serviço da NF3e.
...
flowchart TD
    ...
    B --> C["UniNFe lê consStatServNF3e"]
...
|---|---|---|---|
| Pedido de consulta | Pasta de envio | `<identificador>-ped-sta.xml` | Arquivo criado pelo ERP para consultar a disponibilidade do webservice de NF3e. |
...
Esse arquivo contém a resposta da SEFAZ para a UF e o ambiente consultados. O ERP deve analisar o status e o motivo retornados para decidir se pode iniciar ou continuar as operações de NF3e.
