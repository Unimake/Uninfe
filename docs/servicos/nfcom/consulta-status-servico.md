# Consulta status de serviço da NFCom

A consulta status de serviço da NFCom permite que o ERP verifique se o webservice de NFCom da SEFAZ está disponível para determinada UF e ambiente. Ela deve ser usada antes de operações fiscais ou sempre que houver suspeita de indisponibilidade no serviço.

Esta consulta não envia NFCom, não autoriza documento e não gera XML de distribuição. O resultado serve para orientar o ERP, o suporte e o usuário sobre a disponibilidade do webservice.

## Quando usar

Use este serviço quando:

- O ERP precisa confirmar se o webservice de NFCom está operando.
- O envio de NFCom falhou por suspeita de indisponibilidade da SEFAZ.
...
Para consultar a situação de uma NFCom específica pela chave de acesso, use a [consulta de situação da NFCom](consulta-situacao.md).
...
```text
consStatServNFCom-ped-sta.xml
```

O conteúdo do XML deve usar a estrutura de consulta status de serviço da NFCom:
...
|---|---|
| `versao` | Versão do leiaute da consulta de status da NFCom. |
...
2. O UniNFe lê o XML e identifica a consulta status de serviço da NFCom.
...
flowchart TD
    ...
    B --> C["UniNFe lê consStatServNFCom"]
...
|---|---|---|---|
| Pedido de consulta | Pasta de envio | `<identificador>-ped-sta.xml` | Arquivo criado pelo ERP para consultar a disponibilidade do webservice de NFCom. |
...
Esse arquivo contém a resposta da SEFAZ para a UF e o ambiente consultados. O ERP deve analisar o status e o motivo retornados para decidir se pode iniciar ou continuar as operações de NFCom.
